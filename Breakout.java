...
/** Runs the Breakout program. */
	...
	public void run() {
		...
		boardSetup();	//sets up the initial board state
		addMouseListeners();
		play();
	...
	private void checkForCollision(){
		...
		if(ball.getY() >= APPLICATION_HEIGHT - 2 * BALL_RADIUS) //ball hits bottom of the screen
		{
			lives --;
			remove(livesLabel);
			updateLivesLabel();
		...
		if(collider != null && collider != livesLabel && collider != scoreLabel){ //ensures that ball doesn't interact with labels
			vy = - vy;
			if (collider != paddle){
				...
				getNewvx();
				combo = 1; //resets combo to 1 when ball hits pad
			}
		}
	...
	public void mouseMoved(MouseEvent e){
		if(e.getX() >= PADDLE_WIDTH/2 && e.getX() <= APPLICATION_WIDTH - PADDLE_WIDTH/2){ //if mouse is on the play window
			paddle.setLocation(e.getX() - PADDLE_WIDTH / 2, yPaddle); //track paddle with middle of the mouse
		}
		else if(e.getX() < PADDLE_WIDTH/2){	//if mouse is close to or off the left side of the window
			paddle.setLocation(0, yPaddle); //set paddle to edge of window
		}
		else {
